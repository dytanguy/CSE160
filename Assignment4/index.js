...
void main() {
  ...
  v_lightPos = vec3(u_ModelMatrix * vec4(u_lightPos, 1.0));

  vec3 lightDir = normalize(u_lightPos - vec3(v_vertPos));
  v_spotEffect = dot(normalize(-lightDir), normalize(u_spotDirection));
}`;
...
void main() {
  ...
  if (v_spotEffect >= u_spotCosCutoff) {
    if (u_shadingLevel > 1){
      fragColor += (1. - shadowAmnt(projPoint)) * v_diffuse;
    } else {
      fragColor += v_diffuse;
    }
  }
...
let gld = {
  ...
  u_spotDirection: null,
  u_spotCosCutoff: null,
};
...
let spotlightPosition = new Vector3([37.5, 50, 37.5]);
let spotlightDirection = new Vector3([0, -1, 0]);



let lightAngle = 0;
let lightRadius = 50;
let dynamicLightPos = new Vector3([0, 30, 0]);

let lightColor = new Vector4([1.0, 1.0, 1.0, 1.0]);
...
function main() {
  ...
  document.getElementById("toggleNormalColoring").onclick = () => {
    showNormals = !showNormals;
  };

  document.getElementById("toggleSpotlight").onclick = () => {
...
function getShadowMap(gl){
  ...
  let sphereMatrix = new Matrix4().setTranslate(0, 30, 0);
  gl.uniformMatrix4fv(shadow_gld.u_ModelMatrix, false, sphereMatrix.elements);

  drawSphere(gl, sphereMatrix, [1.0, 1.0, 1.0], {
    a_Position: shadow_gld.a_Position,
    a_Normal: shadow_gld.a_Normal
  }, {
    u_FragColor: gld.u_FragColor, 
    u_ModelMatrix: shadow_gld.u_ModelMatrix,
    u_NormalMatrix: shadow_gld.u_NormalMatrix 
  });
...
function renderScene(gl){
  ...
  gl.uniform1i(gld.u_shadingLevel, lightingEnabled ? 2 : 0);

  gl.uniform1i(gld.u_showNormals, showNormals);

  gl.uniform3f(gld.u_spotDirection, 0.0, -1.0, 0.0); 
  gl.uniform1f(gld.u_spotCosCutoff, Math.cos(Math.PI / 6)); 

  if (spotlightOn) {
    gl.uniform1f(gld.u_spotCosCutoff, Math.cos(Math.PI / 6));
    gl.uniform3f(gld.u_lightPos, ...spotlightPosition.elements);
    gl.uniform3f(gld.u_spotDirection, ...spotlightDirection.elements);
  } else {
    gl.uniform1f(gld.u_spotCosCutoff, -1.0);
    gl.uniform3f(gld.u_spotDirection, 0.0, -1.0, 0.0);
  }
  ...
  light.camera.eye = dynamicLightPos;
  light.camera.at = new Vector3([0, 0, 0]); 
  light.camera.viewMatrix.setLookAt(
    ...light.camera.eye.elements,
    ...light.camera.at.elements,
    ...light.camera.up.elements
  );
  ...
  wObj.render(gl, gld.a_Position, gld.a_UV, gld.u_ModelMatrix, gld.u_NormalMatrix);

  gl.uniform4f(gld.u_FragColor, 1.0, 0.0, 0.0, 1.0);
  ...
  gl.uniformMatrix4fv(gld.u_ViewMatrix, false, camera.viewMatrix.elements);
  const manualLight = document.getElementById("manualLightToggle").checked;

  if (manualLight) {
    let sliderValue = parseFloat(document.getElementById("lightOrbitSlider").value);
    lightAngle = sliderValue / 100;
  } else {
    lightAngle += 0.01;
    document.getElementById("lightOrbitSlider").value = Math.floor(lightAngle * 100);
  }

  document.getElementById("angleValue").innerText = lightAngle.toFixed(2);


  dynamicLightPos = new Vector3([
  ...
  gl.uniform3f(gld.u_lightPos, ...dynamicLightPos.elements);
  ...
  gl.uniform4f(gld.u_illumination, ...lightColor.elements);


  const lightMatrix = new Matrix4().setTranslate(...dynamicLightPos.elements).scale(0.5, 0.5, 0.5);
  const lightNormMatrix = new Matrix4(lightMatrix).invert().transpose();

  let spotDir = new Vector3([0, 0, 0]).sub(dynamicLightPos).normalize();
  gl.uniform3f(gld.u_spotDirection, ...spotDir.elements);


  gl.uniformMatrix4fv(gld.u_NormalMatrix, false, lightNormMatrix.elements);
  gl.uniform1i(gld.u_colorSrc, 0);
  gl.uniform4f(gld.u_FragColor, 1.0, 1.0, 0.0, 1.0);
  drawSphere(gl, lightMatrix, [1.0, 1.0, 0.0], {
